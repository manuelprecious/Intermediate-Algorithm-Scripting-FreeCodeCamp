...
Note: You have to use the arguments objects.
...
function destroyer(arr) {
    ...
    const testParameters = [...arguments].slice(1);
    const firstParam = [...arguments[0]]
    let firstParamLength = firstParam.length;
    
    for (let i = 0; i < firstParamLength; i++){
        if(testParameters.indexOf(firstParam[i]) === -1){
            newArr.push(firstParam[i])
        }
    }
